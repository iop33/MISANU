"""
Results analysis and reporting for GrVRP-PCAFS experiments.

Generates tables and statistics for the scientific paper.
"""
...
def save_results_csv(results: List[Dict], filepath: str):
    ...
    keys = ['instance_name', 'best_distance', 'avg_distance', 'std_distance',
            'worst_distance', 'n_routes', 'avg_time', 'feasible']
...
def save_comparison_csv(gvns_results: List[Dict], 
                         ...
                         filepath: str):
    """Save comparison between GVNS and reference algorithm."""
    keys = ['instance_name', 'gvns_best', 'ref_best', 'gap_%',
            'gvns_avg', 'ref_avg', 'gap_avg_%']
    
    rows = []
    for g, r in zip(gvns_results, reference_results):
...
def compute_summary_stats(all_results: Dict[str, List[Dict]]) -> str:
    """Compute and format summary statistics across all instance sets."""
    lines = [
        "\n" + "=" * 70,
        "SUMMARY STATISTICS",
        "=" * 70,
...
def save_solution_details(solution, filepath: str):
    """Save detailed solution to file."""
    from solution import evaluate_solution
    
    eval_data = evaluate_solution(solution)
    
    data = {
        'total_distance': eval_data['total_distance'],
        'n_routes': eval_data['n_routes'],
        'feasible': eval_data['feasible'],
        'routes': [r.nodes for r in solution.routes],
        'route_distances': [e['distance'] for e in eval_data['route_evaluations']],
        'route_durations': [e['duration'] for e in eval_data['route_evaluations']],
    }
